...
import { Calendar, Clock, MapPin, Video, UserCheck, AlertCircle, FileText, CheckCircle2, XCircle, Ban, Stethoscope } from "lucide-react";
...
const statusBadgeStyles: Record<string, string> = {
  ...
  Rescheduled: "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20",
  "Checked In": "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20",
  "In Consultation": "bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20",
  Completed: "bg-teal-500/10 text-teal-600 dark:text-teal-400 border-teal-500/20",
  Cancelled: "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20",
  Rejected: "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20",
  "No Show": "bg-gray-500/10 text-gray-600 dark:text-gray-400 border-gray-500/20",
};
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-8 rounded-3xl bg-[var(--card-bg)] border border-[var(--border-light)] shadow-lg">
        <div>
          ...
          <h1 className="text-3xl font-bold font-heading text-[var(--text-main)]">My Appointments</h1>
          <p className="text-xs text-[var(--text-secondary)] mt-1">
            Track confirmed time slots, admin scheduling instructions, and digital prescriptions.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 p-8 rounded-3xl bg-[var(--card-bg)] border border-[var(--border-light)]">
          <Calendar size={48} className="mx-auto text-[var(--text-secondary)] mb-4" />
          <h3 className="text-xl font-bold text-[var(--text-main)] font-heading">No Appointments Found</h3>
        ...
        <div className="space-y-6">
          {appointments.map((appt) => (
            <div
              key={appt._id}
              className="rounded-3xl bg-[var(--card-bg)] border border-[var(--border-light)] p-6 md:p-8 shadow-sm hover:shadow-lg transition-all space-y-6"
            >
              {/* Top Row: Doctor Info & Status */}
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-[var(--border-light)]">
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[var(--text-secondary)] font-mono">({appt.appointmentId})</span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border ${
                      statusBadgeStyles[appt.status] || "bg-gray-500/10 text-gray-600"
                    }`}
                  >
                    ● {appt.status}
                  </span>

                  {(appt.status === "Pending Approval" || appt.status === "Requested") && (
                    <button
                      onClick={() => handleCancel(appt._id)}
                      className="px-3.5 py-1.5 rounded-xl border border-red-500/30 text-red-500 text-xs font-semibold hover:bg-red-500/10 transition-all"
                    >
                      Cancel Request
                    </button>
                  )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                <div className="p-4 rounded-2xl bg-[var(--bg-main)] border border-[var(--border-light)] space-y-1">
                  <span className="text-[var(--text-secondary)] block">Confirmed Date & Time</span>
                  <div className="flex items-center gap-2 text-[var(--text-main)] font-semibold">
                    <Calendar size={14} className="text-[var(--accent-primary)]" />
                    <span>
                      {appt.confirmedDate ? `${appt.confirmedDate} at ${appt.confirmedTime}` : "Pending Admin Assignment"}
                    </span>
                  </div>
                </div>

                <div className="p-4 rounded-2xl bg-[var(--bg-main)] border border-[var(--border-light)] space-y-1">
                  <span className="text-[var(--text-secondary)] block">Consultation Mode</span>
                  <div className="flex items-center gap-2 text-[var(--text-main)] font-semibold">
                ...
                <div className="p-4 rounded-2xl bg-[var(--bg-main)] border border-[var(--border-light)] space-y-1">
                  <span className="text-[var(--text-secondary)] block">Patient</span>
                  <div className="flex items-center gap-2 text-[var(--text-main)] font-semibold">
              ...
              <div className="text-xs space-y-1">
                ...
                <p className="text-[var(--text-main)] bg-[var(--bg-main)] p-3 rounded-xl border border-[var(--border-light)]">
                  {appt.symptoms}
                </p>
              </div>
              ...
              {appt.status === "Completed" && (
                <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 space-y-3 text-xs">
                  ...
                  {appt.diagnosis && (
                    <div>
                      <span className="font-semibold text-[var(--text-secondary)]">Clinical Diagnosis:</span>
                      <p className="text-[var(--text-main)] font-medium mt-0.5">{appt.diagnosis}</p>
                    </div>
                  )}
                  {appt.prescription && appt.prescription.length > 0 && (
                    <div>
                      <span className="font-semibold text-[var(--text-secondary)]">Prescribed Medicines:</span>
                      <ul className="mt-1 space-y-1">
                        {appt.prescription.map((p, idx) => (
                          <li key={idx} className="bg-[var(--card-bg)] p-2 rounded-lg border border-emerald-500/20 text-[var(--text-main)] font-mono">
